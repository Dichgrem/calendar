...
│   │   ├── lib/
│   │   │   ├── api.ts          #   REST API 客户端（纯 fetch，无队列）
│   │   │   ├── date-format.ts
│   │   │   ├── lunar.ts        #   农历计算
│   │   │   ├── colors.ts
│   │   │   └── utils.ts
...
启用 WAL 模式。`SetMaxOpenConns(25)`（1 writer + 24 readers）支持并发读写。
...
> 其他设置项（language、firstDayOfWeek、dateFormat、showLunarCalendar、showEventTime、defaultCalendarId）
> 保存于浏览器 localStorage，无需服务端存储。
...
`logger.Debug()`。日志文件存储在 `data/logs/` 目录。
