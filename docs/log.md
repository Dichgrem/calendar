...
> 所有日志通过 `log/slog` 统一输出，同时写入三个目标：
> - **stderr**（终端 / `docker logs` 查看）
> - **内存环形缓冲区**（2000 行，`GET /api/logs` 查看）
> - **日志文件**（`<数据目录>/logs/server-YYYY-MM-DD.log`，10MB/天硬上限，超限自动旋转，7 天清理）
>
...
日志会写入 `<DATABASE_URL 父目录>/logs/server-YYYY-MM-DD.log`。
单文件 10MB 上限，超限时自动重命名为 `.1.log`，每日最多 20MB。
超过 7 天的日志自动清理。

环境变量 `LOG_DIR` 可覆盖日志目录，设为 `-` 则关闭文件日志。
