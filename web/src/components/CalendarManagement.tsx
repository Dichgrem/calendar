...
interface CommonCalendar {
  ...
  descriptionEn: string; url: string; color: string;
}

const COMMON_CALENDARS: CommonCalendar[] = [
  { id: "cn-holidays", name: "中国节假日", nameEn: "Chinese Holidays", description: "法定节假日及调休补班", descriptionEn: "Public holidays", url: "https://cdn.jsdelivr.net/npm/chinese-days/dist/holidays.ics", color: "#ef4444" },
  { id: "cn-festival", name: "节日纪念日", nameEn: "Festivals", description: "传统节日、纪念日", descriptionEn: "Traditional festivals", url: "https://yangh9.github.io/ChinaCalendar/cal_festival.ics", color: "#f59e0b" },
  { id: "cn-solar-term", name: "二十四节气", nameEn: "24 Solar Terms", description: "立春、雨水、惊蛰等", descriptionEn: "Solar terms", url: "https://yangh9.github.io/ChinaCalendar/cal_solarTerm.ics", color: "#10b981" },
];
...
export function CalendarManagement({ calendars }: CalendarManagementProps) {
  ...
  const startEditCal = (cal: Calendar) => { setEditingCal(cal.id); setEditName(cal.name); setEditColor(cal.color); };
  const saveCalEdit = async () => {
    if (!editingCal) return;
    await api.calendars.update(editingCal, { name: editName, color: editColor });
    queryClient.invalidateQueries({ queryKey: ["calendars"] }); setEditingCal(null);
  };
  const deleteCalendar = async (id: string) => {
    if (!window.confirm(t("settings.confirmDelete"))) return;
    setDeleting(id);
    try { await api.calendars.remove(id); queryClient.invalidateQueries({ queryKey: ["calendars"] }); queryClient.invalidateQueries({ queryKey: ["events"] }); }
    catch { setImportError(t("settings.deleteFailed")); } finally { setDeleting(null); }
  };
  const handleExportCal = (id: string, name: string) => {
    const a = document.createElement("a"); a.href = api.ics.exportUrl(id); a.download = `${name}.ics`; a.click();
  };
  const toggleExportCal = (id: string) => setExportSelected((p) => { const n = new Set(p); n.has(id) ? n.delete(id) : n.add(id); return n; });
  const toggleExportAll = () => calendars && setExportSelected((p) => p.size === calendars.length ? new Set() : new Set(calendars.map((c) => c.id)));

  const handleImportCommon = async (cal: CommonCalendar) => {
    setImporting((p) => new Set(p).add(cal.id)); setImportError(null);
    try {
      const res = await api.ics.fetchUrl(cal.url);
      const { preview, content } = (res as any).data;
      await api.ics.import({ content, calendarName: cal.name, color: cal.color, sourceUrl: cal.url, selectedUids: preview.items.map((i: any) => i.uid), overwrite: false });
      queryClient.invalidateQueries({ queryKey: ["calendars"] }); queryClient.invalidateQueries({ queryKey: ["events"] });
    } catch (err) { setImportError(err instanceof Error ? err.message : t("import.importFailed")); }
      ...
      {exportOpen && (
        <div className="mb-3 p-3 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-900/50">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={toggleExportAll} className="text-xs text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300">
          ...
          <div className="flex gap-1">
            <Button size="sm" onClick={() => { calendars?.filter((c) => exportSelected.has(c.id)).forEach((c) => handleExportCal(c.id, c.name)); setExportOpen(false); }} disabled={exportSelected.size === 0} className="h-7 text-xs">{t("settings.exportSelected")} ({exportSelected.size})</Button>
            <Button variant="outline" size="sm" onClick={() => setExportOpen(false)} className="h-7 text-xs">{t("settings.cancel")}</Button>
          </div>
        </div>
      )}
      ...
      {commonCalOpen && (
        <div className="mb-3 space-y-1.5">
          {COMMON_CALENDARS.map((cal) => (
            <div key={cal.id} className="flex items-center gap-2.5 p-2 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-900/50">
              ...
              <Button variant="outline" size="sm" onClick={() => handleImportCommon(cal)} disabled={importing.has(cal.id) || importedCommonIds.has(cal.id)} className="h-7 text-xs shrink-0">
                {importing.has(cal.id) ? t("settings.importing") : importedCommonIds.has(cal.id) ? t("settings.imported") : t("settings.importBtn")}
              </Button>
            </div>
          ))}
          ...
          <div key={cal.id}
            ...
            {editingCal === cal.id ? (
              <>
                ...
                <button onClick={saveCalEdit} aria-label={t("settings.save")} className="size-7 flex items-center justify-center rounded-lg hover:bg-green-100 dark:hover:bg-green-900/30 text-green-600"><Check className="size-4" weight="bold" /></button>
                <button onClick={() => setEditingCal(null)} aria-label={t("settings.cancel")} className="size-7 flex items-center justify-center rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-400"><X className="size-4" weight="bold" /></button>
              </>
            ) : (
              <>
