...
func handlePull(w http.ResponseWriter, r *http.Request) {
	...
	logger.Debug("[sync] GET pull user=%s", perm.UserID)
	lastSeq := 0
	...
	if err == nil {
		...
		for rows.Next() {
			...
			case "created", "updated":
				row := fetchRecord(r.TableName, r.RecordID, perm.UserID)
				if row != nil {
					if r.Op == "created" {
...
func handlePush(w http.ResponseWriter, r *http.Request) {
	perm := middleware.GetPermission(r)
	logger.Debug("[sync] POST push user=%s", perm.UserID)
	var req pushBody
...
func fetchRecord(table string, id string, userID string) map[string]interface{} {
	switch table {
	case "calendars":
		var r struct {
		...
		err := db.DB.QueryRow(
			`SELECT c.id, c.name, c.color, c.source_url, c.source_type, c.owner_id, c.created_at, c.updated_at, c.last_modified
			 FROM calendars c JOIN calendar_members cm ON c.id = cm.calendar_id
			 WHERE c.id=? AND cm.user_id=?`, id, userID,
	...
	case "events":
		...
		err := db.DB.QueryRow(
			`SELECT e.id, e.calendar_id, e.title, e.description, e.start_at, e.end_at, e.all_day, e.rrule,
			        e.color, e.location, e.parent_id, e.original_date, e.deleted, e.created_at, e.updated_at, e.last_modified
			 FROM events e JOIN calendar_members cm ON e.calendar_id = cm.calendar_id
			 WHERE e.id=? AND cm.user_id=?`, id, userID,
	...
	case "event_overrides":
		...
		err := db.DB.QueryRow(
			`SELECT o.id, o.parent_id, o.original_date, o.override_start, o.override_end, o.override_title, o.deleted, o.last_modified
			 FROM event_overrides o
			 JOIN events e ON o.parent_id = e.id
			 JOIN calendar_members cm ON e.calendar_id = cm.calendar_id
			 WHERE o.id=? AND cm.user_id=?`, id, userID,
